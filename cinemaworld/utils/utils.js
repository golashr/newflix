...
const verifyToken = (request, response, next) => {
  ...
  // Check if cinema is undefined
  if (typeof bearerHeader !== 'undefined') {
    // Split at the space
    const cinemaTokens = bearerHeader.split(' ');

    // Get token from array
    const cinemaToken = cinemaTokens[1];

    // Set the token
    request.token = cinemaToken;
...
const verifyAccessToken = (request, response, next) => {
  ...
  const cinemaHeader = request.headers.x-access-token;

  // Check if cinema is undefined
  if (typeof cinemaHeader !== 'undefined') {
    if (cinemaHeader === 'sjd1HfkjU83ksdsm3802k') {
      request.authData = authData;
    ...
    // Split at the space
    // const cinemaTokens = cinemaHeader.split(' ');
