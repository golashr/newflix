    ...
    (err, movies) => {
      if (err) {
        ...
        return errorResponse(
          response,
          '<h1>film World service failed while getting movie.</h1>'
        );
      } else {
        movies.forEach(movie => {
          logger.info(
            `[+] film World service retrieved movie. ${movie.ID}, ${movie.title}, ${movie.description}, ${movie.imageUrl}, ${movie.broadcaster}, ${movie.charges}`
          );
        });
        logger.info(
          `[+] The movie exists with the given ID film World service ${movies}`
        );
        return successResponse(
          response,
          '<h1>The movie exists with the given ID film World service.</h1>',
          movies
        );
      }
    }
...
exports.getMovies = (request, response) => {
  ...
  Movie.find((err, movies) => {
    if (err) {
      ...
      return errorResponse(
        response,
        '<h1>film World service failed while getting movies.</h1>'
      );
    } else {
      movies.forEach(movie => {
        logger.info(
      ...
      logger.info(
        `[+] film World service returns all listed movies. ${movies}`
      );
      return successResponse(
        response,
        '<h1>film World service returns all listed movies.</h1>',
        movies
      );
    }
  });
...
exports.addMovie = (request, response) => {
  logger.info(
    '[+] Movies to addMovie from Cinemaworld Service ${request.query}'
  );
  data = request.query;
  logger.info(
    `[+] film World service added. ${data.ID}, ${data.title}, ${data.description}, ${data.imageUrl}, ${data.broadcaster}, ${data.charges}`
  );
  var movie = new Movie();
  movie.ID = data.ID;
  ...
  movie.save(err => {
    if (err) {
      ...
      return errorResponse(
        response,
        '<h1>film World service failed while adding movie.</h1>'
      );
    } else {
      logger.info(
        `[+] film World service added movie successfully. ${movie.title}`
      );
      return successResponse(
        response,
        '<h1>film World service returns all listed movies.</h1>',
